...
- DNS는 <storng>네임 서버 (Name Server)</strong> 를 운영하는 장치이다.
...
- 입력한 텍스트가 'URL'이면, 브라우저 엔진에서 네트워크 스레드를 통해 '네트워크 호출'을 수행한다.

<br/>

### Step 2. 네트워크 호출

    (위에 정리해놓은 <a href="">네트워크 사전 지식</a>이 필요하다.)

- 사용자가 주소창에 도메인 네임이 포함된 URL (여기서는 <code>www.naver.com</code>) 를 입력하면

- 클라이언트 (사용자의 컴퓨터) 는 hosts 파일에서 그 도메인 네임 (<code>naver.com</code>) 에 대응하는 IP 주소 정보가 있는지 확인한다.

- 만일 없다면, 클라이언트는 네임 서버에게 도메인의 IP 주소를 질의하는 요청 패킷을 보낸다.

- 네임 서버는 자신의 데이터베이스 목록에 해당 IP 주소를 찾아 클라이언트에게 응답 패킷을 보낸다.

<br/>

- 클라이언트는 이렇게 알아낸 IP 주소(<code>125.209.222.142</code>)를 바탕으로 TCP 소켓을 통해 네이버 서버에 HTTP Request를 보낸다.

- HTTP Request를 받은 네이버 서버는 HTTP Reply를 클라이언트에 보낸다.

- 네이버 서버로부터 응답을 받은 클라이언트의 브라우저는 페이지를 띄우는데 필요한 데이터 (HTML, CSS, Javascript, 이미지 파일 등) 를 얻었다.

<br/>

### Step 3. 렌더링 작업

- 네트웨크 스레드는 이렇게 얻은 데이터를 우선 검사한다.

  - 악성 바이러스가 있는지 없는지..

  - (만일 다운로드 파일 형식의 데이터가 응답될 경우, 다운로드 매니저에게 이를 전달한다.)

- 모든 검사가 끝나면 UI 스레드는 렌더링 엔진에게 해당 웹 페이지를 화면에 띄울 것을 요청한다.
