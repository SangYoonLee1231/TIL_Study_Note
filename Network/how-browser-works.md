...
- 정리한 내용을 바탕으로 <strong>블로그 포스트</strong>를 작성했습니다. 👉<a href="https://velog.io/@sylagape1231/주소창에-naver.com을-치면-일어나는-일을-쉽게-이해해보자">여기</a>를 클릭하시면 확인할 수 있습니다.
...
- DOM 트리 생성과정에서 브라우저는 문서에 있는 에러 등, 닫는 태그가 없는 에러 등을 자동으로 처리해준다.
...
- 전체적인 화면 안에서 렌더 트리 각 노드의 위치, 크기 (너비와 높이) 를 계산하는 과정

- 노드의 위치는 (x, y) 좌표계를 사용한다.  
  ( 최상위 노드의 위치 = (0, 0) )

- 랜더 트리의 <strong>루트부터 아래로 내려가면서</strong> 계산을 진행한다.
...
- 초기 배치 이후 DOM 노드가 추가되거나 변경되면, 불필요한 레이아웃이 발생하지 않도록 전체가 아닌 일부만 다시 배치된다.
  ...
  (이를 '<strong>Reflow</strong>'라 한다. 아래에서 다룬다.)
...
- 렌더 트리를 순회하며 <strong>레이어</strong>를 만들고, 레이어의 배경, 테두리, 텍스트, 그려지는 순서, 레이어 간의 순서 등 그려지는 과정을 기록한다.

- 렌더 트리의 각 노드를 <strong>화면의 실제 픽셀로 변환</strong>하여(이를 '래스터화'라 한다) 브라우저 화면에 그려준다.

<br/><br/>

### 리플로우 (Reflow) 와 리페인트 (Repaint)

- 사용자의 조작으로 인해 화면의 요소(의 크기나 위치 등)에 변경이 생긴다면, 브라우저는 레이아웃 (Layout) 작업과 페인트 (Paint) 작업을 다시 수행해여 사용자에게 바뀐 화면을 보여주어야 할 것이다.

- 레이아웃 (Layout) 작업이 다시 일어나는 것을 <strong>리플로우 (Reflow)</strong>, 페인트 (Paint) 작업이 다시 일어나는 것을 <strong>리페인트 (Repaint)</strong>라고 한다.

<br/>

- 위치나 크기의 변화같이 화면의 구조가 달라지는 변화는 Reflow와 Repaint 두 과정이 모두 (순차적으로) 일어난다.

- 색상 변화와 같이 화면의 구조가 바뀌지 않는 스타일의 변화는 Repaint 과정만 일어난다.
...
> - <a href="https://basemenks.tistory.com/292">브라우저 렌더링 과정 (파싱-렌더-레이아웃-페인팅)</a> (블로그 포스트)
