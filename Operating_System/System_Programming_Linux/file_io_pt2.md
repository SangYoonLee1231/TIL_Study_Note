...
- 라이브러리 버퍼는 내가 쓰는 라이브러리에서 관리하는 버퍼를 말한다.

  - fread → 데이터 전달 → buffer에 자연스럽게 카피

  - 만일 버퍼가 없다면 출력 시 바로 커널로 전달되어 장치에 나타나야 함
...
- 상황을 보고 버퍼링 모드를 사용해야 할 지 말지 스스로 판단해야 한다. (소프트웨어 아키택체가 해야할 일)

  - 시스템을 잘 이해하고 있어야 가능한 일
