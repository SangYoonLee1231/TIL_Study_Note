# 리눅스 파일 관리 시스템 Part 4 (프로세스 생성 원리 : <code>fork</code>, <code>exec</code> 시스템 콜)
...
- 리눅스에선 프로세스 생성하는 방식이 특이하다.
  ...
  - 리눅스에서 새로운 프로세스를 만들려면 `fork` 시스템 콜을 사용하여 만든다.
...
- 부모 프로세스가 fork를 하고 나면 부모 프로세스는 fork로부터 return 해야 한다.

  - child도 마찬가지로 fork으로부터 돌아오는 시점부터 실행이 시작됨

- parent와 child 프로세스는 겉으로 봐선 똑같기 때문에, OS에서 parent와 child를 구분시켜주는 역할을 수행한다.

  - return 값 : **child = 0, parent = 막 태어난 child의 pid (0보다 큰 양수 값, 보통 parent보다 큰 값)**

  - parent와 child가 각각 본인이 해야 할 일을 구별해서 할 수 있게 된다.
...
- wait 시스템 콜이란?
  ...
  - 부모, 자식 프로세스가 만들어지면 각자 자신의 길을 가는 독립적인 프로세스로 인식된다.
    ...
    - OS에서도 둘을 똑같은 프로세스로 대한다.

  - 그러나, 경우에 따라 부모가 자식을 만들어두고 잠시 본인의 동작을 일시중지하고 있다가 자식이 잘 실행되고 종료될 때까지 기다릴 수도 있다.
...
- 처음에 프로세스가 존재하다가 fork하는 순간 자식 프로세스가 생성된다.

- 그 후 각각 fork으로부터 리턴값을 받는다.
...
- exit 시스템 콜이란?
  ...
  - 부모가 자식 프로세스를 (fork 시스템 콜을 통해) 생성한 후, wait 시스템 콜로 인해 본인의 동작이 중지된 채 자식 프로세스가 끝나기를 기다리는 상황일 때,
...
- exit 시스템 콜의 동작 과정
  ...
  - (이때 마침 wait을 할 때 어떤 변수의 주소를 전달해준다 → OS에게 어떤 정보를 이 주소에 저장해 달라)
...
- 시스템 콜의 변종 (variation)
  ...
  - (exec-v를 이용하면 하고 싶을 일을 대부분 할 수 있다.)
...
- init을 fork하면 그 순간에는 init의 클론 (자식들) 생성

  → exec(getty) → getty 내용으로 바뀜

- getty → 누군가 접근을 시도히먄 login으로 바뀜 (exec) → login에 성공하면 shell process로 변신 (exec) (**모두 다 같은 프로세스이다.**)
...
2. 그 후 Shell은 해당 명령어를 실핼하기 위해 프로세스를 하나 생성한다.

3. 그 때 fork를 통해 Shell의 똑같은 자식 프로세스가 하나 만들어진다.

   - 우리 눈에 보이진 않고, OS 내부에서 이미지만 똑같은 자료구조체가 만들어진다.

4. Shell이 exec을 통해 실행시킬 프로그램으로 변신한다. (ex : 아래 한글)

   - 아래 한글의 parent process는 Shell이다.

- 이처럼 Shell과 그 뒤에 만들어지는 응용 프로그램은 서로 부모-자식 관계가 된다.
...
- `$ .a/out` : shell, a.out 프로세스 생성
  ...
  - shell 내부에 자식을 fork 후 wait하는 코드가 있다.

- `$ cat myfile` : cat은 파일을 표준 출력에 띄워주는 역할을 하는 명령어이다.

  - 역시 cat이라는 프로세스가 만들어진다.
...
- CPU를 한 순간에 한 프로세스가 잠깐 쓰고 그 이후는 다음 프로세스가 쓰는 방식으로 CPU를 서로 공유한다.
...
- 따라서 CPU가 다음 프로세스로 넘어갈 때 기존에 작업한 내용들을 어딘가에 저장해두어야 한다. 그곳이 PCB이다.
...
- 이 때, 부모 프로세스가 3번째 데이터 c를 읽고 파일에 write하기 직전에 CPU Time Up이 일어날 수 있다.

  - 언제 프로세스가 CPU를 뺏길 지 알 수가 없는 것이다.
...
- 이렇게 데이터를 쓰고 읽는데 순서가 뒤엉킬 수 있는 상황을 **Race Condition**이라 부른다.
  ...
  - **Race Condition** : 실행시켜봐야 알 수 있는 예측 불허의 상황

<br/>

#### 해결 방법

- 이를 방지하기 위한 여러 메커니즘(프로세스 동기화(싱크로나이제이션), 뮤텍스)을 배울 것이다.

- 서로 다른 프로세스 간에 협력 작업을 하기 위한 여러 방법이 있다.

  - 이를 IPC (= Inter Process Communication)라 한다.
...
- pipe는 프로세스 간에 데이터를 잘 교환하도록 OS에서 제공하는 특별한 자료구조(원형 큐)이다.

- pipe는 원형 큐이기 때문에 OS 안에서 제공하는 특수한 메모리에서 제공하는 자료형인데,

- 실제로 사용자들은 이를 파일이라 생각하고 생각하도록

- 사용자에게 제공되는 인터페이스 형식은 파일의 형식으로,

- 내부 구조는 메모리 buffer의 형식으로 제공되는 것이 pipe이다.

<br/>

- pipe는 시스템에서 이름을 전혀 모르는 프로세스들 끼리도 pipe를 사용할 수 있게끔 이름을 붙여서 제공하는 named pipe가 있고

- 이름 없이 부모 자식간에는 서로 암묵적으로 만들어서 쓰는 unnamed pipe 또는 anonymous pipe가 있다.
