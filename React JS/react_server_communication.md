...
- 1️⃣ 2️⃣ 3️⃣ 순서대로 실행
  ...
  - 2️⃣의 처리 과정은 시간이 걸리기에, 비동기적으로 3️⃣이 동시에 실행된다.

- 2️⃣의 `setDiaryList(data)` → 문제의 코드

  - 상태변화함수이므로 리액트 컴포넌트 상태를 변경시킨다. 즉, 페이지가 재실행된다.

  - 그렇게 되면 3️⃣의 진행 역시 초기화되고, 실행 흐름은 다시 1️⃣로 돌아간다.

- 위의 과정이 무한으로 반복하게 되어 이로 인해 무한 리렌더링 오류가 발생한다.
