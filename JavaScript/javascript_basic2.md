...
- <code>prompt</code>
  ...
  - 사용자에게 메서지와 입력 필드를 함께 보여주는 모달 창(Modal Window)을 띄운다.

  - [확인]을 누르면 사용자가 입력 필드에 입력한 문자열이 반환되고,  
    [취소] 또는 Esc를 누르면 <code>null</code>이 반환된다.

  - 반환된 값은 아래처럼 지정한 변수에 할당하여 활용할 수 있다.
...
- <code>confirm</code>
  ...
  - 사용자에게 메세지 그리고 눌러야 할 [확인] 또는 [취소] 버튼을 모달 창(Modal Window)으로 보여준다.
...
- 어떤 값의 자료형이 다른 자료형으로 변환되는 과정을 '형 변환(Type Conversion)'이라 한다.

* 형 변환엔 크게 2가지 종류가 있다.

  * 자동 형 변환 : 상황에 따라 값의 자료형이 자동으로 바뀌는 과정

  * 명시적 형 변환 : 전달받은 값을 의도를 갖고 원하는 자료형으로 직접 변환해주는 과정
