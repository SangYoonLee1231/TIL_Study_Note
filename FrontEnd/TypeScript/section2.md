...
- TS에 object라는 타입은 그냥 이 값이 객체인 것 외에는 아무런 정보도 없는 타입이기 때문에
...
- 객체의 타입을 정의할 때는 객체의 모든 프로퍼티들의 타입까지 구조적으로 정의하는 방식인 **객체 리터럴 타입**을 사용한다.
...
- readonly 키워드를 붙여주면 프로퍼티 값이 바뀌는 것을 막아준다.
