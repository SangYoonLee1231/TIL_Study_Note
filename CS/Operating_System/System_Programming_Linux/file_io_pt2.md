...
- <a href="https://github.com/SangYoonLee1231/TIL/blob/main/Operating_System/System_Programming_Linux/file_io_pt2.md#%ED%8C%8C%EC%9D%BC-%EA%B4%80%EB%A0%A8-%EB%9D%BC%EC%9D%B4%EB%B8%8C%EB%9F%AC%EB%A6%AC-%ED%95%A8%EC%88%98">파일 관련 라이브러리 함수</a>
<!-- - <a href=""></a> -->
...
- <code>cp -r /sp-resource/.vim\* .</code> : .vim으로 시작하는 모든 디렉토리를 복사
  ...
  - <code>.vim\*</code> : .vim으로 시작하는 모든 파일

  - <code>.</code> : 현재 위치(로 복사)
...
- 캐싱 기법은 모든 계층에 다 적용된다.

  - 하드디스크 보다 더 빠른 SSD의 속도에 비하면 하드디스크는 너무 느린 장치

    - SSD의 일부 공간에 하드디스크의 저장 내용을 올려둠

    - 하드디스크에 접근하는 횟수 감소 → 성능 개선

  - 느린 장치에 있는 데이터를 빠른 장치 위에 (복제본을) 올려 이를 활용하는 테크닉 : **캐싱 기법**

  - 커널 안에도 일종의 캐시 역할을 하는 공간이 있다. → **버퍼 (Buffer)**

<br/>

- 파일 스트림과 파일 디스크립터는 1대1로 대응한다.

  - 파일 스트림 안에는 파일과 관련된 여러 정보들이 있지만

  - 그 중 중요한 정보로 파일 디스크립터가 몇 번인지를 기록하고 있다.
...
- 버퍼링의 종류
  ...
  - 유저 레벨의 버퍼링 : <strong>라이브러리 버퍼 (library buffer)</strong>
...
- 라이브러리 버퍼는 **내가 쓰는 라이브러리에서 관리하는 버퍼**를 말한다.

  - `fread` → 데이터 전달 → buffer에 자연스럽게 카피

  - 만일 버퍼가 없다면, 출력 시 바로 커널로 전달되어 장치에 나타나야 한다.

  - 버퍼가 있으면, 엔터가 들어오거나 버퍼에 있는 내용이 일정량 다 차야 커널에 전달된다.

  <br/>

  - 버퍼를 사용할 때, (출력을 예로 듦) 내가 write 하면 버퍼링 하지 말고 바로 커널로 내려보내서 장치에 나타나게 하라 → **Unbuffering**

  - 시스템 콜로 매번 내려보내면 시간 오래 걸림, 비용 문제 발생 → 그래서 버퍼를 쓰는 것이다.
...
- **Full Buffering** : 버퍼 용량이 꽉 차야 다음 과정(ex: I/O)이 수행
  ...
  - 시스템 콜 횟수를 현격히 줄일 수 있음

  - 버퍼가 꽉 찰 때까지는 파일 I/O가 일어나지 않는다. (눈에 나타나지 않음)
...
- **Line Buffering** : 한 줄이 찰 때마다 커널로 내려보내라
  ...
  - `getchar()` (키보드에서 문자 하나씩 받아들일 때) 문제점
...
- 상황을 보고 버퍼링 모드를 사용해야 할 지 말지 스스로 판단해야 한다. (소프트웨어 아키택처가 해야할 일)

  - 시스템을 잘 이해하고 있어야 가능한 일이다.

- 데이터 안정성을 추구하려면 `fflush()` 시스템 콜을 자주 호출하여 버퍼에 있는 데이터를 밑으로 내려보내야 한다.

<br/>

- 리눅스에서의 기본 버퍼 모드 (디폴트 값)
...
- 에러 코드를 잘 처리하는 것도 매우 중요한 일 중 하나이다.

- 시스템 콜이든 라이브러리 콜이든 호출하면 리턴값을 확인하는 습관을 가질 것 (에러 처리 잘하기)
...
- 버퍼 캐시의 목적 : **디스크의 접근 횟수를 줄이기 위해서**
  ...
  - b/c : 디스크에 접근에서 데이터를 읽어오는 시간이 (CPU 입장에서) 오래 걸리기 때문이다.
...
- 버퍼 캐시는 데이터를 읽을 때에도 유용하다.

  - 디스크까지 내려갈 필요 없이 버퍼 캐시에서 데이터를 읽으면 되므로

- 버퍼 캐시는 데이터를 쓸 때도 유용하다.
...
- **Delayed Write** : 디스크에 데이터가 write 되는 시점이 실제 write 시점보다 약간 늦게 일어난다.
  ...
  - 성능 개선 효과가 있으나, 갑자기 전원이 나가면 데이터가 증발할 위험 또한 존재한다.
...
- <code>size_t fread (void *ptr, size_t size, size_t nitems, FILE *stream)</code>
  ...
  - 파일로부터 데이터를 읽어서 지정된 크기만큼 버퍼에 저장하는 함수이다. (read는 파일 기준)
...
- <code>size_t fwrite (void *ptr, size_t size, size_t nitems, FILE *stream)</code>
  ...
  - `fread`와 반대로 버퍼에 저장된 데이터를 파일로 쓰는 함수이다. (write는 파일 기준)

  - 각 인자의 의미

    - <code>void \*ptr</code> : 데이터를 쓸 버퍼의 주소

    - <code>size_t size</code> : 쓸 데이터의 크기 (객체 유닛 하나의 크기)
...
- 커널에서도 마찬가지 현상이 일어난다.
  ...
  <img src="../img/buffer_system_calls.png">
