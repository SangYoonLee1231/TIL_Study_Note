...
- 막판에 알 수 없는 에러로 인해 큰 고통을 겪었다. 아직도 이 문제는 해결하지 못한 상태이다.
  ...
  - 이 문제 때문에 언어 안가리고 공식 문서, 깃허브, 그 외 수많은 페이지를 돌아다니며 해결법을 찾아봤으나 큰 소득은 없었다.  
    결국 코드숨 대표님께 여쭤봐서 실마리를 하나 더 얻을 수 있었다. 하지만 그럼에도 문제를 해결하진 못헀다.

  - 꼬리에 꼬리를 무는 다른 에러가 발생해 잠시 낙심이 왔다. 다행이도 구글링으로 해결할 수 있었지만. (NVM use 오류)
