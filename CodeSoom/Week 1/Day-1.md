...
- 처음부터 끝까지 다 처음 배우는 내용이라 생소했지만 그렇기에 많은 것을 배울 수 있어 재밌었다.

  하지만 진도가 느려 과제를 커녕 개발환경 구축도 못 끝내고 하루가 끝나버렸다..

- 막판에 알 수 없는 에러가 내 인내심을 제대로 시험했다. 결국 오늘 안에 해결하지 못했다.

  - 적어도 나 외에 이런 문제를 겪은 사람이 이 세상 어딘가엔 있을텐데, 왜 구글링 해도 유용한 정보가 이렇게 안 나오는 거지?

  - 심지어 Stack Overflow에서도 관련 에러에 대한 정보를 찾을 수 없었다.  
    (다행이도 이 문제가 '버전 호환성'과 관련된 것이란 실마리는 얻었지만)

  - 이 문제 때문에 언어 안가리고 공식 문서, 깃허브, 그 외 수많은 페이지를 돌아다니며 해결법을 찾아봤으나 결국 못 찾음.  
    코드숨 대표님께도 여쭤봐 작은 실마리를 하나 얻었으나 그 이상의 도움은 받지 못함.

  - 꼬리에 꼬리를 무는 에러로 인해 정말 고통스러웠음 (NVM use 오류 봤을 땐 할많하않)

  - 내 짧은 코딩 인생 역대급 난제다. 내일이 오면 이 문제를 해결할 수 있을까?

- 내가 이 교육을 제대로 활용은 할 수 있을까? 아님 지금의 나한텐 안 맞는 교육인가..? 어쩌면 내일도 PR 못 날릴 수 있을텐데.. 환불해야되나? 그래도 이제 시작인데.. 좀만 더 고생해볼까
